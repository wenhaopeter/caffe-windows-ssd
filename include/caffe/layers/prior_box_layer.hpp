...
namespace caffe {
	...
	* 这层的作用就是对不同位置的每个特征点产生不同大小的default box，
	*  这些box的大小、形状由prototxt的prior_box_param来控制
	...
	*/
	...
	protected:
		...
		*/
		...
		vector<float> min_sizes_;   //prototxt文件中的min_size
		vector<float> max_sizes_;
		vector<float> aspect_ratios_;   //高宽比
		bool flip_;    //是否翻转
		int num_priors_;
		bool clip_;     //是否修剪
		...
		int img_w_;  
		int img_h_;
		float step_w_;   //表示的是的与原图的缩放比例
		float step_h_;

		float offset_;   //priorbox在特征图中wh的偏移
	};
}
