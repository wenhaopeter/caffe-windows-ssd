...
namespace caffe {
	...
	AnnotatedDataLayer<Dtype>::AnnotatedDataLayer(const LayerParameter& param)
		: BasePrefetchingDataLayer<Dtype>(param),  //这个类继承于BaseDataLayer<Dtype>和 InternalThread
		reader_(param) {  //DataReader<AnnotatedDatum> 类型 对reader_初始化，并开始读取数据
	}
	...
	template <typename Dtype>
	void AnnotatedDataLayer<Dtype>::DataLayerSetUp(  //某一层的参数设置在这个函数里实现
		const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
		//data_param()是proto中的
		const int batch_size = this->layer_param_.data_param().batch_size();  //通过layer_param_返回层的参数
		...
		//在该文件里存贮的是label name 和lable id，在LabelMap format.格式中
		...
		//可以获取要读的数据信息，并返回anno_atum，后面再根据Datum来reshape
		...
		LOG(INFO) << "output data size: " << top[0]->num() << ","
			...
			<< top[0]->width();
		// label
		if (this->output_labels_) {  //在anno_datum中和anno_data_param 都没有type这个选项
	...
	void AnnotatedDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
		...
		const TransformationParameter& transform_param =
			this->layer_param_.transform_param();
		AnnotatedDatum& anno_datum = *(reader_.full().peek()); //从数据哭中读取数据
															   // Use data_transformer to infer the expected blob shape from anno_datum.
		vector<int> top_shape =
			this->data_transformer_->InferBlobShape(anno_datum.datum());
		this->transformed_data_.Reshape(top_shape);
		...
		if (this->output_labels_ && has_anno_type_) {
			...
			if (anno_type_ == AnnotatedDatum_AnnotationType_BBOX) { //进入这个分支
				label_shape[0] = 1;
				label_shape[1] = 1;
				label_shape[3] = 8;
				if (num_bboxes == 0) {
	...
	//对层进行注册
	INSTANTIATE_CLASS(AnnotatedDataLayer);  //类的名称
	REGISTER_LAYER_CLASS(AnnotatedData);     //层的名称
