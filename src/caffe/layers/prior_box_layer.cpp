...
namespace caffe {
	...
	void PriorBoxLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
		...
		for (int i = 0; i < prior_box_param.aspect_ratio_size(); ++i) {
			...
			bool already_exist = false;  //检查高宽比是否已经存在
										 // 保证每种长宽比唯一存在
		...
		// 如果有设置max_size，则num_priors+1，表示最大正方形边长(sqrt(min_size*max_size))
		...
		//LOG(WARNING) << " aspect_ratios:" << aspect_ratios_.size() << " num_priors_:" << num_priors_ 
		//	<< "  max_size:" << prior_box_param.max_size_size() << " min_size:" << prior_box_param.min_size_size();
		clip_ = prior_box_param.clip();
		if (prior_box_param.variance_size() > 1) {
			// variance_size必需为4  方差
	...
	void PriorBoxLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
		...
		if (img_h_ == 0 || img_w_ == 0) {
			img_width = bottom[1]->width();  //在prototxt文件中没有设置img_h和img_w
		...
		float step_w, step_h;  //将step的值赋给step_w, step_h;
